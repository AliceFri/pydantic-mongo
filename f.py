...
def to_query(data: Dict[str, Any]) -> Dict[str, Any]:
    """
    将 data字典 转换为 query 参数
    """
    query = {}
    for k, v in data.items():
        ...
        if k == 'id':
            ...
            if isinstance(v[0], Enum):
                query[k] = {'$in': list(map(lambda x: x.value, v))}
            else:
                query[k] = {'$in': v}
...
class _BaseModel(pydantic.BaseModel):
    ...
    def to_query(self) -> dict:
        ...
        return to_query(self.dict())


@cache
def create_model(
    baseclass, include: tuple = None, exclude: tuple = None, optional: tuple = None
) -> _BaseModel:
    def _parse_name():
    ...
    def _get_field(key, item):
        default = _get_default(key, item)
        try:  # 尝试进行递归创建model
            if issubclass(item.type_, _BaseModel):
                # For Typing.List
                if getattr(item.outer_type_, "_name", "") == 'List':
                    return (
                        list[
                            create_model(
                                item.type_,
...
def opt_create_model(
    baseclass, include: tuple = None, exclude: tuple = None
) -> _BaseModel:
